#!/usr/bin/env python3
"""
Test script for TRC rate regression models.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test TRC rate regression models.")
    parser.add_argument(
        "--model-dir",
        required=True,
...
def instantiate_torch_model(metadata: Dict[str, object], input_dim: int, output_dim: int) -> torch.nn.Module:
    ...
    params: Dict[str, Any] = metadata["model_params"]  # type: ignore
...
def main() -> None:
    ...
    if target_mode != "rate":
        raise ValueError(f"metadata.target_mode={target_mode} is not 'rate'; use test_trc_value.py instead.")
    ...
    target_mean = mean[target_indices] if "mean" in scalers else scalers["target_mean"]
    target_std = std[target_indices] if "std" in scalers else scalers["target_std"]
