#!/usr/bin/env python3
"""
Train rate-based regression models with TRC-only output targets.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train rate-based regression models with TRC-only targets.")
...
def configure_output_dirs(model_name: str, config_path: Path) -> Tuple[Path, Path]:
    ...
    if "/trial_" in model_name:
        ...
        trial_info = parts[1]
        # 修复目录命名逻辑，避免重复添加后缀
        if base_model_name.endswith("-trc-rate"):
            # 如果base_model_name已经包含了-trc-rate后缀，直接使用
            output_dir = Path("scripts") / "outputs" / base_model_name / trial_info
        else:
            # 否则添加-trc-rate后缀
            output_dir = Path("scripts") / "outputs" / f"{base_model_name}-trc-rate" / trial_info
    else:
        # 如果是普通运行，使用原始命名方式
        output_dir = Path("scripts") / "outputs" / f"{model_name}-trc-rate"
...
def main() -> None:
    ...
    rate_targets, base_targets, base_columns = compute_rate_targets(df, TRC_TARGET_COLUMNS)
