#!/usr/bin/env python3
"""
Train regression models with OTHER (non-TRC) output targets.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train regression models with OTHER (non-TRC) targets.")
...
def configure_output_dirs(model_name: str, config_path: Path) -> Tuple[Path, Path]:
    ...
    if "/trial_" in model_name:
        ...
        trial_info = parts[1]
        # 修复目录命名逻辑，避免重复添加后缀
        if base_model_name.endswith("-other-value"):
            # 如果base_model_name已经包含了-other-value后缀，直接使用
            output_dir = Path("scripts") / "outputs" / base_model_name / trial_info
        else:
            # 否则添加-other-value后缀
            output_dir = Path("scripts") / "outputs" / f"{base_model_name}-other-value" / trial_info
    else:
        # 如果是普通运行，使用原始命名方式
        output_dir = Path("scripts") / "outputs" / f"{model_name}-other-value"
...
def main() -> None:
    ...
    if cfg.model_type in {"XGBOOST", "LIGHTGBM", "CATBOOST"}:
        raise NotImplementedError("Tree-based models not yet implemented for OTHER targets training")
