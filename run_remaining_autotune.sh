...
# 每个模型都包含4个任务：TRC值回归、TRC比率回归、OTHER值回归、OTHER比率回归
# 所有模型的trial数量从100减少到80
...
# 1. 运行LSTM OTHER值回归调参
echo "$count/$total: 运行LSTM OTHER值回归调参..."
nohup python scripts/bayes_autotune_other.py --model-type LSTM --n-trials 80 > lstm_other_value.log 2>&1 &
PID1=$!
echo "LSTM OTHER值回归调参已在后台运行，PID: $PID1"

# 等待第一个任务完成
wait $PID1
echo "LSTM OTHER值回归调参已完成"
((count++))

# 2. 运行LSTM OTHER比率回归调参
echo "$count/$total: 运行LSTM OTHER比率回归调参..."
nohup python scripts/bayes_autotune_other_rate.py --model-type LSTM --n-trials 80 > lstm_other_rate.log 2>&1 &
PID2=$!
echo "LSTM OTHER比率回归调参已在后台运行，PID: $PID2"
...
nohup python scripts/bayes_autotune_trc_rate.py --model-type MLP --n-trials 80 \
  --base-config models/configs/mlp_config.yaml \
  --grid-config models/configs/mlp_bayes.yaml > mlp_trc_rate.log 2>&1 &
PID4=$!
echo "MLP TRC比率回归调参已在后台运行，PID: $PID4"
...
nohup python scripts/bayes_autotune_other.py --model-type MLP --n-trials 80 \
  --base-config models/configs/mlp_config.yaml \
  --grid-config models/configs/mlp_bayes.yaml > mlp_other_value.log 2>&1 &
PID5=$!
echo "MLP OTHER值回归调参已在后台运行，PID: $PID5"
...
nohup python scripts/bayes_autotune_other_rate.py --model-type MLP --n-trials 80 \
  --base-config models/configs/mlp_config.yaml \
  --grid-config models/configs/mlp_bayes.yaml > mlp_other_rate.log 2>&1 &
PID6=$!
echo "MLP OTHER比率回归调参已在后台运行，PID: $PID6"
...
# 7. 运行MLP HIS TRC值回归调参
echo "$count/$total: 运行MLP HIS TRC值回归调参..."
nohup python scripts/bayes_autotune_trc.py --model-type MLP_WITH_HISTORY --n-trials 80 \
...
PID7=$!
echo "MLP HIS TRC值回归调参已在后台运行，PID: $PID7"
...
# 8. 运行MLP HIS TRC比率回归调参
echo "$count/$total: 运行MLP HIS TRC比率回归调参..."
nohup python scripts/bayes_autotune_trc_rate.py --model-type MLP_WITH_HISTORY --n-trials 80 \
  --base-config models/configs/mlp_with_history_config.yaml \
  --grid-config models/configs/mlp_with_history_grid.yaml > mlphis_trc_rate.log 2>&1 &
PID8=$!
echo "MLP HIS TRC比率回归调参已在后台运行，PID: $PID8"
...
# 9. 运行MLP HIS OTHER值回归调参
echo "$count/$total: 运行MLP HIS OTHER值回归调参..."
nohup python scripts/bayes_autotune_other.py --model-type MLP_WITH_HISTORY --n-trials 80 \
  --base-config models/configs/mlp_with_history_config.yaml \
  --grid-config models/configs/mlp_with_history_grid.yaml > mlphis_other_value.log 2>&1 &
PID9=$!
echo "MLP HIS OTHER值回归调参已在后台运行，PID: $PID9"
...
# 10. 运行MLP HIS OTHER比率回归调参
echo "$count/$total: 运行MLP HIS OTHER比率回归调参..."
nohup python scripts/bayes_autotune_other_rate.py --model-type MLP_WITH_HISTORY --n-trials 80 \
  --base-config models/configs/mlp_with_history_config.yaml \
  --grid-config models/configs/mlp_with_history_grid.yaml > mlphis_other_rate.log 2>&1 &
PID10=$!
echo "MLP HIS OTHER比率回归调参已在后台运行，PID: $PID10"
...
nohup python scripts/bayes_autotune_trc_rate.py --model-type RNN --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/rnn_bayes.yaml > rnn_trc_rate.log 2>&1 &
PID12=$!
echo "RNN TRC比率回归调参已在后台运行，PID: $PID12"
...
nohup python scripts/bayes_autotune_other.py --model-type RNN --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/rnn_bayes.yaml > rnn_other_value.log 2>&1 &
PID13=$!
echo "RNN OTHER值回归调参已在后台运行，PID: $PID13"
...
# 14. 运行RNN OTHER比率回归调参
echo "$count/$total: 运行RNN OTHER比率回归调参..."
nohup python scripts/bayes_autotune_other_rate.py --model-type RNN --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/rnn_bayes.yaml > rnn_other_rate.log 2>&1 &
PID14=$!
echo "RNN OTHER比率回归调参已在后台运行，PID: $PID14"
...
nohup python scripts/bayes_autotune_trc.py --model-type GRU --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/gru_bayes.yaml > gru_trc_value.log 2>&1 &
PID15=$!
echo "GRU TRC值回归调参已在后台运行，PID: $PID15"
...
nohup python scripts/bayes_autotune_trc_rate.py --model-type GRU --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/gru_bayes.yaml > gru_trc_rate.log 2>&1 &
PID16=$!
echo "GRU TRC比率回归调参已在后台运行，PID: $PID16"
...
nohup python scripts/bayes_autotune_other.py --model-type GRU --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/gru_bayes.yaml > gru_other_value.log 2>&1 &
PID17=$!
echo "GRU OTHER值回归调参已在后台运行，PID: $PID17"
...
# 18. 运行GRU OTHER比率回归调参
echo "$count/$total: 运行GRU OTHER比率回归调参..."
nohup python scripts/bayes_autotune_other_rate.py --model-type GRU --n-trials 80 \
  --base-config models/configs/rnn_config.yaml \
  --grid-config models/configs/gru_bayes.yaml > gru_other_rate.log 2>&1 &
PID18=$!
echo "GRU OTHER比率回归调参已在后台运行，PID: $PID18"
