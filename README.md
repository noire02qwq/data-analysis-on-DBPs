...
| `models/` | Trainable model code (`mlp_regressor.py`, `lstm_regressor.py`), YAML configs under `models/configs/`, and generated artifacts in `models/outputs/<model_name>/`. |
| `prompt-draft/` | Vibe-coding task briefs that describe each experiment in natural language. |
| `scripts/` | All runnable scripts (`fill_missing.py`, `align_time.py`, `train_regression.py`, `test_regression.py`, shared regression utilities, plus a script-level README). |
...
4. **Evaluate / plot predictions**  
   `python scripts/test_regression.py --model-dir models/outputs/<model_name>`
...
- Each training run writes checkpoints, scalers, plots, and metadata under `models/outputs/<model_name>/` so testing can reuse the exact scalers and splits.
