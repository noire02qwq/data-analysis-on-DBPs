#!/usr/bin/env python3
"""
Train regression models with TRC-only output targets.
"""
...
# TRC-only target columns
TRC_TARGET_COLUMNS: List[str] = [
    "TRC-PPL1",
    "TRC-PPL2",
]
...
def save_scalers(path: Path, mean: np.ndarray, std: np.ndarray) -> None:
    """Save feature scalers to a compressed numpy file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train regression models with TRC-only targets.")
...
def configure_output_dirs(model_name: str, config_path: Path) -> Tuple[Path, Path]:
    ...
    if "/trial_" in model_name:
        ...
        trial_info = parts[1]
        # 修复目录命名逻辑，避免重复添加后缀
        if base_model_name.endswith("-trc-value"):
            # 如果base_model_name已经包含了-trc-value后缀，直接使用
            output_dir = Path("scripts") / "outputs" / base_model_name / trial_info
        else:
            # 否则添加-trc-value后缀
            output_dir = Path("scripts") / "outputs" / f"{base_model_name}-trc-value" / trial_info
    else:
        # 如果是普通运行，使用原始命名方式
        output_dir = Path("scripts") / "outputs" / f"{model_name}-trc-value"
...
def get_trc_feature_and_target_indices(columns: List[str]) -> Tuple[List[int], List[int]]:
    column_to_idx = {col: idx for idx, col in enumerate(columns)}
    target_indices: List[int] = []
    for col in TRC_TARGET_COLUMNS:
        if col not in column_to_idx:
            missing = set(TRC_TARGET_COLUMNS) - set(column_to_idx)
...
def main() -> None:
    ...
    feature_indices, target_indices = get_trc_feature_and_target_indices(columns)
    ...
    if cfg.model_type in {"XGBOOST", "LIGHTGBM", "CATBOOST"}:
        raise NotImplementedError("Tree-based models not yet implemented for TRC-only training")
    ...
    metadata = {
        ...
        "target_columns": TRC_TARGET_COLUMNS,
        "split_boundaries": {
